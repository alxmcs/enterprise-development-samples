    ...
    : CrudControllerBase<BookDto, BookCreateUpdateDto, int>(crudService, logger)
{
    ...
    {
        logger.LogInformation("{method} method of {controller} is called with {id} parameter", nameof(GetAuthorBooks), GetType().Name, authorId);
        try
        {
            var res = await crudService.GetAuthorBooks(authorId, HttpContext.RequestAborted);
